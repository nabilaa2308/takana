...
class kategori
{
    ...
    {
        // prepare query sql
        $query = $this->connect->prepare("Select * from kategori where id = :id");
    ...
    {
        try {
            $query = $this->connect->prepare("SELECT * FROM kategori ORDER BY id DESC");
            $query->execute(); // execute the query
    ...
    function create()
    {
        $data = $_POST;
        try {
            $query = $this->connect->prepare("INSERT INTO kategori (nama) VALUES (:nama)");
    ...
    function update()
    {
        $data = $_POST;
        try {
            $query = $this->connect->prepare("UPDATE kategori SET nama = :nama WHERE id = :id");
    ...
    function delete()
    {
        $id = $_GET['id'] ?? NULL;
        try {
            $query = $this->connect->prepare("DELETE FROM kategori WHERE id = :id");
